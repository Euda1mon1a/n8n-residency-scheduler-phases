...
class EnhancedFacultyAssignmentEngine:
    ...
    def find_clinic_template(self, activity: str, activity_type: str,
        ...
        # In a real scenario, this would lookup from a passed clinic_template_lookup
        # For this standalone version, we'll return a generic template or need to pass lookup
        # We'll assume generic for now to keep it simple, or add lookup to init if needed
        # Adding simplified logic here:
        return {
            'id': 'tmpl_generic',
            'name': activity if not is_substitution else 'Leave Supervision Override'
        }

    def get_half_day_info(self, half_day_id: str, assignment: Dict) -> Dict:
        """Get half-day information (simplified)."""
        # In real usage, this parses the half_day_id or looks it up
        # For simulation, we'll extract date if possible or use a placeholder
        # Assuming half_day_id might contain date info or we just use a fixed date for testing
        # Let's try to extract if it looks like a date, otherwise default
        return {
        ...
        for index, half_day_id in enumerate(half_day_ids):
            ...
            if faculty_selection:
                ...
                faculty_assignment = {
                    ...
                    # Add fields needed for Phase 7 validation
                    'Faculty': [faculty_selection['faculty']['id']],
                    'Attending Clinic Templates': [clinic_template['name']],
                    'Half-Day of the Week of Blocks': [half_day_id]
                }
...
def run_phase3(master_assignments: List[Dict], faculty_data: List[Dict], 
               clinic_templates: List[Dict], absence_data: Dict) -> Dict:
    """Wrapper to run Phase 3 logic"""
    
    # 1. Build Enhanced Faculty Lookup
    enhanced_faculty_lookup = {}
    faculty_absences = absence_data.get('facultyAbsences', {})
    ...
    for faculty in faculty_data:
        ...
        enhanced_faculty_lookup[faculty_id] = {
            ...
            'absenceCalendar': faculty_absences.get(faculty_id, {}),
            'currentWorkload': 0
        }
        
    # 2. Initialize Engine
    engine = EnhancedFacultyAssignmentEngine(
        enhanced_faculty_lookup,
        faculty_absences,
        SUPERVISION_RATIOS,
        SPECIALTY_REQUIREMENTS
    ...
    for assignment in master_assignments:
        results = engine.generate_faculty_assignment(assignment)
        all_assignments.extend(results)
        
    return {
        'assignments': all_assignments,
        'substitutions': engine.absence_substitutions,
        'gaps': engine.coverage_gaps,
        'workload': engine.faculty_workload
    }
